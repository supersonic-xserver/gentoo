...
EGIT_REPO_URI="https://github.com/supersonic-xserver/sonicd.git"
inherit git-r3
...
migrate_locale() {
	...
	if [[ ! -L ${locale_conf} && ! -e ${locale_conf} ]]; then
		...
		if [[ -e ${envd_locale} ]]; then
			...
			cat > "${locale_conf}" <<-EOF
				# This file has been created by the sys-apps/sonicd ebuild.
...
pkg_preinst() {
	...
	if ! use boot && has_version "sys-apps/sonicd[gnuefi(-)]"; then
